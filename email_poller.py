...
def detect_intent_and_fields(text: str):
    ...
    # Customer name (minimal extraction)
    m = re.search(r"\bmy name is\s+([a-zA-Z][a-zA-Z\-']{1,30})\b", t)
    if m:
        fields["customer_name"] = m.group(1).strip().title()

    # Phone (very loose; demo only)
                        ...
                        if th["flags"].get("hold_created"):
                            ...
                            if extras_missing:
                                ...
                                if not res.get("ok"):
                                    msg_fail = (
                                        "Hi,\n\n"
                                        "I tried to create the provisional hold, but something failed on our side.\n"
                                        f"Error: {res.get('error','unknown')}\n\n"
                                        "Please reply with your date again (today/tomorrow or YYYY-MM-DD) and I’ll try immediately.\n\n"
                                        "Warm regards,\nMarina\nBlueMarlin Tours Curaçao\n"
                                    )
