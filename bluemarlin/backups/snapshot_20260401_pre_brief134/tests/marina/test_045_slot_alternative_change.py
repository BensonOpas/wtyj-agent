...
def test_marina_agent_header():
    ...
    assert "Last modified: Brief" in source, "marina_agent.py header must reference Brief"
    print("  T5 PASS: marina_agent.py header updated")

def test_email_poller_header():
    """T6: email_poller.py file header says Brief 045."""
    from agents.marina import email_poller
    source = inspect.getsource(email_poller)
    assert "Last modified: Brief" in source, "email_poller.py header must reference Brief"
    print("  T6 PASS: email_poller.py header updated")
