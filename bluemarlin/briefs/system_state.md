...
Outcome: complete — 14/14 new tests pass, 656 total pass, 7 pre-existing failures unchanged. Two containers running on VPS simultaneously. Architectural flaw discovered during deployment: Dockerfile COPY bluemarlin/ /app/ bakes BlueFinn's runtime secrets (azure_refresh_token.txt, email_thread_state.json, platform.env) into the image because docker build doesn't honor .gitignore. Not blocking for THIS proof (Adamus's EMAIL_ADDRESS="" means the poller exits before touching baked-in token, and volume mounts override baked-in client.json), but IS a showstopper for any real multi-client deployment where a second client actually uses email. Deferred to Brief 147: add .dockerignore excluding bluemarlin/config/* from build context.
