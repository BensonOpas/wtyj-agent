...
Decision: Replaced the one-line free-text `common_sense_knowledge.marina_persona` field with a structured `agent_persona` section containing 10 discrete fields: tone, language_register, greeting_style, closing_style, brand_voice_rules (array), topics_allowed (array), topics_refused (array), small_talk, escalation_tone, freeform_notes. New `marina_agent._build_agent_persona_block()` helper assembles a multi-section prompt block from these fields with backward-compat fallback to the legacy string. Migrated BlueFinn and Adamus client.jsons with content-appropriate per-client voice. Added `business.operating_mode` ("full_booking" | "qualify_and_escalate") as a human-readable alias for `features.booking_flow`. Added `agent_persona` to `marina_agent._SKIP_TOP_LEVEL` to prevent double injection via the auto-iterator in `_build_client_context()`. Migrated `dashboard/api.py` draft-email endpoint to use the same helper so Marina's real reply path and the dashboard's email drafter share one source of truth.
Outcome: complete — 19/19 new tests pass, 700 total pass, 7 pre-existing failures unchanged. Live verification inside both containers confirmed distinct structured personas active: BlueFinn gets "warm, calm, practical, guest-aware" with multilingual Caribbean/charter context; Adamus gets "warm, casual, beachy" with one-exclamation-max rule and restaurant topic boundaries. Zero cross-contamination between the two personas. Each new client can now meaningfully customize voice at 10 axes instead of one free-text blob. Round 1 reviewer caught 5 issues (all patched), round 2 caught 3 more (2 fixed inline, 1 led to reverting the content_agent skip-list change to preserve the dashboard /config endpoint). One pre-existing test fixed to import `_SKIP_TOP_LEVEL` dynamically so future additions don't re-break it.
