...
# --- Test 3: Semi-escalation sets relay state ---
...
def test_semi_escalation_sets_relay_state(mock_process, mock_sheets):
    ...
    assert len(state["flags"].get("relay_token", "")) == 12
    assert state["flags"].get("relay_question") == "What is the weight limit for jet skis?"
    # Sheets logged
